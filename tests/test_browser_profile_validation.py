...
def test_save_missing_session_404():
    ...
    r = client.post("/api/v1/browser/profiles/save", json={"session_id": "bsess-missing"})
    assert r.status_code == 404
    detail = r.json()["detail"]
    assert detail["code"] == "SESSION_NOT_FOUND"


def test_save_missing_session_id_400():
    from aethos_core.api.main import app

    client = TestClient(app)
    r = client.post("/api/v1/browser/profiles/save", json={"session_id": ""})
    assert r.status_code == 422


def test_save_expired_session_409():
    from aethos_core.api.main import app
    from aethos_core.runtime.browser_profile_store import browser_profile_store
    ...
    try:
        ...
        r = client.post("/api/v1/browser/profiles/save", json={"session_id": sid})
        assert r.status_code == 409
        assert r.json()["detail"]["code"] == "SESSION_NOT_ACTIVE"
        assert "expired" in r.json()["detail"]["detail"].lower()
    finally:
