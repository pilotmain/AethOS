...
def test_connection_revoke_removes_token(vault_env):
    ...
    cid = created["credential"]["credential_id"]
    client.post(f"/api/v1/connections/vercel/credentials/{cid}/revoke")
    assert get_credential_vault().get(cid) is None
    listed = client.get("/api/v1/connections/vercel").json()
    assert listed["connected_methods"]["api_token"] == "missing"
