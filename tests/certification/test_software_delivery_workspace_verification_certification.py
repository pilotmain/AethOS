...
"""FIX 125E — workspace verification certification."""
...
SESSION = "software-delivery-cert-125e"
...
class TestSoftwareDeliveryWorkspaceVerificationCertification:
    def test_governed_workspace_verification_lane(self) -> None:
        resolve_chat_turn(
            "analyze github issue pilotmain/AethOS#105",
            session_id=SESSION,
        ...
        verify = resolve_chat_turn(
            "run workspace verification",
            session_id=SESSION,
            apply_relational_layer=False,
        )
        assert_route_owns(verify, route_id="software_delivery_issue_plan")
        assert verify.intent == "software_delivery_workspace_verification_passed"
        assert "pr_drafting_unblocked" in verify.reply
        ...
        assert "Verification receipts" in timeline.reply
