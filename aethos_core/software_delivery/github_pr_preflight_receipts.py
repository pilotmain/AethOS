...
"""FIX 125G — GitHub PR preflight receipts."""
...
def _store_dir() -> Path:
    root = Path(__file__).resolve().parents[2] / "data" / "software_delivery_github_pr_preflight_receipts"
...
def _path(plan_id: str) -> Path:
    ...
    return _store_dir() / f"{safe}_github_pr_preflight_receipts.json"
...
def clear_for_tests() -> None:
    ...
    if _store_dir().exists():
        for child in _store_dir().glob("*_github_pr_preflight_receipts.json"):
            child.unlink(missing_ok=True)


def list_github_pr_preflight_receipts(*, plan_id: str) -> list[dict[str, Any]]:
...
def record_github_pr_preflight_receipt(
    *,
    plan_id: str,
    phase: str,
    status: str = "preflight_step_success",
    detail: str = "",
    preflight_id: str = "",
    ...
    receipt = {
        ...
        "schema_version": GITHUB_PR_PREFLIGHT_SCHEMA_VERSION,
        "recorded_at": datetime.now(UTC).isoformat(),
    }
    rows = list_github_pr_preflight_receipts(plan_id=plan_id)
    rows.append(receipt)
