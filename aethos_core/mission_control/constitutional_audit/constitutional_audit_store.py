...
"""FIX 160 — durable store for constitutional audit records."""
...
from aethos_core.mission_control.constitutional_audit.constitutional_audit_contract import (
    ...
    MAX_AUDIT_CONTENT_LEN,
    MAX_PERSISTED_AUDIT_RECORDS,
)


def constitutional_audit_records_dir() -> Path:
    root = (
...
def clear_constitutional_audit_records_for_tests() -> None:
    root = constitutional_audit_records_dir()
    if root.exists():
...
def _record_path(record_id: str) -> Path:
    ...
    return constitutional_audit_records_dir() / f"{safe}.json"


def list_constitutional_audit_records(
    *,
    session_id: str | None = None,
    plan_id: str | None = None,
    limit: int = MAX_PERSISTED_AUDIT_RECORDS,
) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    paths = sorted(constitutional_audit_records_dir().glob("*.json"), key=lambda p: p.stat().st_mtime)
    for path in paths:
        try:
...
def append_constitutional_audit_record(
    *,
    session_id: str,
    kind: str,
    content: str,
    ...
    if kind_norm not in AUDIT_RECORD_KINDS:
    ...
    if not text:
        ...
        return None, blockers
    if len(text) > MAX_AUDIT_CONTENT_LEN:
...
def _prune_old_records(*, keep: int = MAX_PERSISTED_AUDIT_RECORDS) -> int:
    paths = sorted(constitutional_audit_records_dir().glob("*.json"), key=lambda p: p.stat().st_mtime, reverse=True)
    removed = 0
