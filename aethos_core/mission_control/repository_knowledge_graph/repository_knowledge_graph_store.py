# SPDX-License-Identifier: Apache-2.0
"""FIX 240 — durable store for repository knowledge graph."""

from __future__ import annotations

import json
import uuid
from datetime import UTC, datetime
from pathlib import Path
from typing import Any

from aethos_core.mission_control.repository_knowledge_graph.repository_knowledge_graph_contract import (
    CODE_MODIFICATION_AUTHORITY_FIX_240,
    CROSS_REPO_AUTHORITY_FIX_240,
    GOVERNANCE_MUTATION_PERFORMED_FIX_240,
    KNOWLEDGE_GRAPH_EXECUTION_FIX_240,
    MAX_PERSISTED_REPOSITORY_KNOWLEDGE_GRAPH_RECORDS,
    MAX_REPOSITORY_KNOWLEDGE_GRAPH_CONTENT_LEN,
    REPOSITORY_AUTHORITY_FIX_240,
    REPOSITORY_KNOWLEDGE_GRAPH_EXECUTABLE,
    REPOSITORY_KNOWLEDGE_GRAPH_ORIGIN,
    REPOSITORY_KNOWLEDGE_GRAPH_RECORD_KINDS,
    REPOSITORY_KNOWLEDGE_GRAPH_RECORD_SCHEMA_VERSION,
)


def repository_knowledge_graph_records_dir() -> Path:
    root = (
        Path(__file__).resolve().parents[3]
        / "data"
        / "mission_control_repository_knowledge_graph"
        / "records"
    )
    root.mkdir(parents=True, exist_ok=True)
    return root


def clear_repository_knowledge_graph_records_for_tests() -> None:
    root = repository_knowledge_graph_records_dir()
    if root.exists():
        for child in root.glob("*.json"):
            child.unlink(missing_ok=True)


def list_repository_knowledge_graph_records(
    *,
    session_id: str | None = None,
    repository_id: str | None = None,
    limit: int = MAX_PERSISTED_REPOSITORY_KNOWLEDGE_GRAPH_RECORDS,
) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    paths = sorted(
        repository_knowledge_graph_records_dir().glob("*.json"),
        key=lambda p: p.stat().st_mtime,
    )
    for path in paths:
        try:
            payload = json.loads(path.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError):
            continue
        if not isinstance(payload, dict):
            continue
        if session_id and str(payload.get("session_id") or "") != session_id:
            continue
        if repository_id and str(payload.get("repository_id") or "") != repository_id:
            continue
        rows.append(payload)
    rows.sort(key=lambda r: str(r.get("recorded_at") or ""))
    if len(rows) > limit:
        rows = rows[-limit:]
    return rows


def append_repository_knowledge_graph_record(
    *,
    session_id: str,
    kind: str,
    content: str,
    repository_id: str | None = None,
    author: str = "operator",
    metadata: dict[str, Any] | None = None,
) -> tuple[dict[str, Any] | None, list[str]]:
    blockers: list[str] = []
    kind_norm = (kind or "").strip().lower()
    if kind_norm not in REPOSITORY_KNOWLEDGE_GRAPH_RECORD_KINDS:
        blockers.append(f"invalid_kind:{kind_norm}")
        return None, blockers

    text = (content or "").strip()
    if not text:
        blockers.append("empty_content")
        return None, blockers
    if len(text) > MAX_REPOSITORY_KNOWLEDGE_GRAPH_CONTENT_LEN:
        blockers.append("content_too_long")
        return None, blockers

    sid = (session_id or "default").strip()[:64] or "default"
    record: dict[str, Any] = {
        "schema_version": REPOSITORY_KNOWLEDGE_GRAPH_RECORD_SCHEMA_VERSION,
        "record_id": f"rkg-{uuid.uuid4().hex[:12]}",
        "recorded_at": datetime.now(UTC).isoformat(),
        "session_id": sid,
        "repository_id": repository_id,
        "kind": kind_norm,
        "author": (author or "operator").strip()[:64] or "operator",
        "content": text,
        "metadata": dict(metadata or {}),
        "executable": REPOSITORY_KNOWLEDGE_GRAPH_EXECUTABLE,
        "repository_authority": REPOSITORY_AUTHORITY_FIX_240,
        "code_modification_authority": CODE_MODIFICATION_AUTHORITY_FIX_240,
        "cross_repo_authority": CROSS_REPO_AUTHORITY_FIX_240,
        "knowledge_graph_execution": KNOWLEDGE_GRAPH_EXECUTION_FIX_240,
        "governance_mutation_performed": GOVERNANCE_MUTATION_PERFORMED_FIX_240,
        "repository_knowledge_graph_memory_only": True,
        "origin": REPOSITORY_KNOWLEDGE_GRAPH_ORIGIN,
    }
    path = repository_knowledge_graph_records_dir() / f"{record['record_id']}.json"
    path.write_text(json.dumps(record, indent=2), encoding="utf-8")
    _prune_old_records()
    return record, blockers


def _prune_old_records(*, keep: int = MAX_PERSISTED_REPOSITORY_KNOWLEDGE_GRAPH_RECORDS) -> int:
    paths = sorted(
        repository_knowledge_graph_records_dir().glob("*.json"),
        key=lambda p: p.stat().st_mtime,
        reverse=True,
    )
    removed = 0
    for path in paths[keep:]:
        path.unlink(missing_ok=True)
        removed += 1
    return removed
