...
"""FIX 193 — durable store for Atlas Trader pilot arc orchestrator."""
...
from aethos_core.mission_control.atlas_trader_pilot_arc_orchestrator.atlas_trader_pilot_arc_orchestrator_contract import (
    ...
    ATLAS_TRADER_PILOT_ARC_ORCHESTRATOR_RECORD_KINDS,
    ATLAS_TRADER_PILOT_ARC_ORCHESTRATOR_RECORD_SCHEMA_VERSION,
    ATLAS_TRADER_REPOSITORY,
    GOVERNANCE_MUTATION_PERFORMED_FIX_193,
    MAX_ATLAS_TRADER_PILOT_ARC_ORCHESTRATOR_CONTENT_LEN,
    MAX_PERSISTED_ATLAS_TRADER_PILOT_ARC_ORCHESTRATOR_RECORDS,
)

_ISSUE_RX = re.compile(r"pilotmain/[\w.-]+#\d+", re.I)


def atlas_trader_pilot_arc_orchestrator_records_dir() -> Path:
    root = (
...
def clear_atlas_trader_pilot_arc_orchestrator_records_for_tests() -> None:
    root = atlas_trader_pilot_arc_orchestrator_records_dir()
    if root.exists():
        for child in root.glob("*.json"):
            child.unlink(missing_ok=True)


def list_atlas_trader_pilot_arc_orchestrator_records(
    *,
    limit: int = MAX_PERSISTED_ATLAS_TRADER_PILOT_ARC_ORCHESTRATOR_RECORDS,
) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    paths = sorted(
        atlas_trader_pilot_arc_orchestrator_records_dir().glob("*.json"),
...
def registered_repo_issue() -> str:
    for record in reversed(list_atlas_trader_pilot_arc_orchestrator_records()):
    ...
    return ATLAS_TRADER_DEFAULT_REPO_ISSUE
...
def append_atlas_trader_pilot_arc_orchestrator_record(
    *,
    session_id: str,
    kind: str,
    content: str,
    ...
    if kind_norm not in ATLAS_TRADER_PILOT_ARC_ORCHESTRATOR_RECORD_KINDS:
    ...
    if not text and kind_norm != "pilot_arc_transition":
        ...
        return None, blockers
    if len(text) > MAX_ATLAS_TRADER_PILOT_ARC_ORCHESTRATOR_CONTENT_LEN:
    ...
    record: dict[str, Any] = {
        ...
        "record_id": f"atpao-{uuid.uuid4().hex[:12]}",
        ...
        "origin": ATLAS_TRADER_PILOT_ARC_ORCHESTRATOR_ORIGIN,
    }
    path = atlas_trader_pilot_arc_orchestrator_records_dir() / f"{record['record_id']}.json"
...
def _prune_old_records(
    *, keep: int = MAX_PERSISTED_ATLAS_TRADER_PILOT_ARC_ORCHESTRATOR_RECORDS
) -> int:
    paths = sorted(
        atlas_trader_pilot_arc_orchestrator_records_dir().glob("*.json"),
        key=lambda p: p.stat().st_mtime,
