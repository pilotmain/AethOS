...
"""FIX 168 — durable store for bounded delivery work package records."""
...
from aethos_core.mission_control.bounded_multi_agent_delivery_work_packages.bounded_multi_agent_delivery_work_packages_contract import (
    ...
    MAX_PERSISTED_WORK_PACKAGES_RECORDS,
    MAX_WORK_PACKAGES_CONTENT_LEN,
    WORK_PACKAGES_RECOMMENDATION_EXECUTABLE,
    WORK_PACKAGES_RECORD_KINDS,
)


def bounded_delivery_work_packages_records_dir() -> Path:
    root = (
...
def clear_bounded_delivery_work_packages_records_for_tests() -> None:
    root = bounded_delivery_work_packages_records_dir()
    if root.exists():
...
def _record_path(record_id: str) -> Path:
    ...
    return bounded_delivery_work_packages_records_dir() / f"{safe}.json"


def list_bounded_delivery_work_packages_records(
    *,
    session_id: str | None = None,
    plan_id: str | None = None,
    limit: int = MAX_PERSISTED_WORK_PACKAGES_RECORDS,
) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    paths = sorted(bounded_delivery_work_packages_records_dir().glob("*.json"), key=lambda p: p.stat().st_mtime)
    for path in paths:
        try:
...
def append_bounded_delivery_work_packages_record(
    *,
    session_id: str,
    kind: str,
    content: str,
    ...
    if kind_norm not in WORK_PACKAGES_RECORD_KINDS:
    ...
    if not text:
        ...
        return None, blockers
    if len(text) > MAX_WORK_PACKAGES_CONTENT_LEN:
    ...
    record: dict[str, Any] = {
        ...
        "record_id": f"wpkg-{uuid.uuid4().hex[:12]}",
...
def _prune_old_records(*, keep: int = MAX_PERSISTED_WORK_PACKAGES_RECORDS) -> int:
    paths = sorted(
        bounded_delivery_work_packages_records_dir().glob("*.json"),
