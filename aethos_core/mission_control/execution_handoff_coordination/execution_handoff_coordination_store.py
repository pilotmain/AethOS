...
"""FIX 167 — durable store for execution handoff coordination records."""
...
from aethos_core.mission_control.execution_handoff_coordination.execution_handoff_coordination_contract import (
    ...
    MAX_HANDOFF_CONTENT_LEN,
    MAX_PERSISTED_HANDOFF_RECORDS,
    EXECUTION_HANDOFF_COORDINATION_RECORD_SCHEMA_VERSION,
)


def execution_handoff_coordination_records_dir() -> Path:
    root = (
...
def clear_execution_handoff_coordination_records_for_tests() -> None:
    root = execution_handoff_coordination_records_dir()
    if root.exists():
...
def _record_path(record_id: str) -> Path:
    ...
    return execution_handoff_coordination_records_dir() / f"{safe}.json"


def list_execution_handoff_coordination_records(
    *,
    session_id: str | None = None,
    plan_id: str | None = None,
    limit: int = MAX_PERSISTED_HANDOFF_RECORDS,
) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    paths = sorted(execution_handoff_coordination_records_dir().glob("*.json"), key=lambda p: p.stat().st_mtime)
    for path in paths:
        try:
...
def append_execution_handoff_coordination_record(
    *,
    session_id: str,
    kind: str,
    content: str,
    ...
    if kind_norm not in HANDOFF_RECORD_KINDS:
    ...
    if not text:
        ...
        return None, blockers
    if len(text) > MAX_HANDOFF_CONTENT_LEN:
    ...
    record: dict[str, Any] = {
        ...
        "record_id": f"handoff-{uuid.uuid4().hex[:12]}",
...
def _prune_old_records(*, keep: int = MAX_PERSISTED_HANDOFF_RECORDS) -> int:
    paths = sorted(
        execution_handoff_coordination_records_dir().glob("*.json"),
