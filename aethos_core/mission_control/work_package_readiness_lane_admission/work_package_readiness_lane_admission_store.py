...
"""FIX 169 — durable store for work package readiness + lane admission records."""
...
from aethos_core.mission_control.work_package_readiness_lane_admission.work_package_readiness_lane_admission_contract import (
    ...
    MAX_LANE_ADMISSION_CONTENT_LEN,
    MAX_PERSISTED_LANE_ADMISSION_RECORDS,
    WORK_PACKAGE_READINESS_LANE_ADMISSION_RECORD_SCHEMA_VERSION,
)


def work_package_readiness_lane_admission_records_dir() -> Path:
    root = (
...
def clear_work_package_readiness_lane_admission_records_for_tests() -> None:
    root = work_package_readiness_lane_admission_records_dir()
    if root.exists():
...
def _record_path(record_id: str) -> Path:
    ...
    return work_package_readiness_lane_admission_records_dir() / f"{safe}.json"


def list_work_package_readiness_lane_admission_records(
    *,
    session_id: str | None = None,
    plan_id: str | None = None,
    limit: int = MAX_PERSISTED_LANE_ADMISSION_RECORDS,
) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    paths = sorted(work_package_readiness_lane_admission_records_dir().glob("*.json"), key=lambda p: p.stat().st_mtime)
    for path in paths:
        try:
...
def append_work_package_readiness_lane_admission_record(
    *,
    session_id: str,
    kind: str,
    content: str,
    ...
    if kind_norm not in LANE_ADMISSION_RECORD_KINDS:
    ...
    if not text:
        ...
        return None, blockers
    if len(text) > MAX_LANE_ADMISSION_CONTENT_LEN:
    ...
    record: dict[str, Any] = {
        ...
        "record_id": f"ladm-{uuid.uuid4().hex[:12]}",
...
def _prune_old_records(*, keep: int = MAX_PERSISTED_LANE_ADMISSION_RECORDS) -> int:
    paths = sorted(
        work_package_readiness_lane_admission_records_dir().glob("*.json"),
