...
"""FIX 154 — durable store for governance resilience records."""
...
from aethos_core.mission_control.governance_resilience.governance_resilience_contract import (
    ...
    MAX_PERSISTED_RESILIENCE_RECORDS,
    MAX_RESILIENCE_CONTENT_LEN,
    RESILIENCE_RECORD_KINDS,
    RESILIENCE_SIMULATION_EXECUTABLE,
)


def resilience_records_dir() -> Path:
    root = Path(__file__).resolve().parents[3] / "data" / "mission_control_governance_resilience" / "records"
    root.mkdir(parents=True, exist_ok=True)
    return root


def clear_governance_resilience_records_for_tests() -> None:
    root = resilience_records_dir()
    if root.exists():
...
def _record_path(record_id: str) -> Path:
    ...
    return resilience_records_dir() / f"{safe}.json"


def list_governance_resilience_records(
    *,
    session_id: str | None = None,
    plan_id: str | None = None,
    limit: int = MAX_PERSISTED_RESILIENCE_RECORDS,
) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    paths = sorted(resilience_records_dir().glob("*.json"), key=lambda p: p.stat().st_mtime)
    for path in paths:
        try:
...
def append_governance_resilience_record(
    *,
    session_id: str,
    kind: str,
    content: str,
    ...
    if kind_norm not in RESILIENCE_RECORD_KINDS:
    ...
    if not text:
        ...
        return None, blockers
    if len(text) > MAX_RESILIENCE_CONTENT_LEN:
    ...
    record: dict[str, Any] = {
        ...
        "record_id": f"grs-{uuid.uuid4().hex[:12]}",
...
def _prune_old_records(*, keep: int = MAX_PERSISTED_RESILIENCE_RECORDS) -> int:
    paths = sorted(resilience_records_dir().glob("*.json"), key=lambda p: p.stat().st_mtime, reverse=True)
    removed = 0
