...
"""FIX 158 — durable store for institutional existential risk records."""
...
from aethos_core.mission_control.institutional_existential_risk.institutional_existential_risk_contract import (
    ...
    MAX_EXISTENTIAL_RISK_CONTENT_LEN,
    MAX_PERSISTED_EXISTENTIAL_RISK_RECORDS,
)


def existential_risk_records_dir() -> Path:
    root = (
...
def clear_institutional_existential_risk_records_for_tests() -> None:
    root = existential_risk_records_dir()
    if root.exists():
...
def _record_path(record_id: str) -> Path:
    ...
    return existential_risk_records_dir() / f"{safe}.json"


def list_institutional_existential_risk_records(
    *,
    session_id: str | None = None,
    plan_id: str | None = None,
    limit: int = MAX_PERSISTED_EXISTENTIAL_RISK_RECORDS,
) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    paths = sorted(existential_risk_records_dir().glob("*.json"), key=lambda p: p.stat().st_mtime)
    for path in paths:
        try:
...
def append_institutional_existential_risk_record(
    *,
    session_id: str,
    kind: str,
    content: str,
    ...
    if kind_norm not in EXISTENTIAL_RISK_RECORD_KINDS:
    ...
    if not text:
        ...
        return None, blockers
    if len(text) > MAX_EXISTENTIAL_RISK_CONTENT_LEN:
    ...
    record: dict[str, Any] = {
        ...
        "record_id": f"ierisk-{uuid.uuid4().hex[:12]}",
...
def _prune_old_records(*, keep: int = MAX_PERSISTED_EXISTENTIAL_RISK_RECORDS) -> int:
    paths = sorted(existential_risk_records_dir().glob("*.json"), key=lambda p: p.stat().st_mtime, reverse=True)
    removed = 0
