...
"""FIX 156 — durable store for institutional identity records."""
...
from aethos_core.mission_control.institutional_identity.institutional_identity_contract import (
    ...
    MAX_IDENTITY_CONTENT_LEN,
    MAX_PERSISTED_IDENTITY_RECORDS,
)


def identity_records_dir() -> Path:
    root = Path(__file__).resolve().parents[3] / "data" / "mission_control_institutional_identity" / "records"
    root.mkdir(parents=True, exist_ok=True)
    return root


def clear_institutional_identity_records_for_tests() -> None:
    root = identity_records_dir()
    if root.exists():
...
def _record_path(record_id: str) -> Path:
    ...
    return identity_records_dir() / f"{safe}.json"


def list_institutional_identity_records(
    *,
    session_id: str | None = None,
    plan_id: str | None = None,
    limit: int = MAX_PERSISTED_IDENTITY_RECORDS,
) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    paths = sorted(identity_records_dir().glob("*.json"), key=lambda p: p.stat().st_mtime)
    for path in paths:
        try:
...
def append_institutional_identity_record(
    *,
    session_id: str,
    kind: str,
    content: str,
    ...
    if kind_norm not in IDENTITY_RECORD_KINDS:
    ...
    if not text:
        ...
        return None, blockers
    if len(text) > MAX_IDENTITY_CONTENT_LEN:
    ...
    record: dict[str, Any] = {
        ...
        "record_id": f"iid-{uuid.uuid4().hex[:12]}",
...
def _prune_old_records(*, keep: int = MAX_PERSISTED_IDENTITY_RECORDS) -> int:
    paths = sorted(identity_records_dir().glob("*.json"), key=lambda p: p.stat().st_mtime, reverse=True)
    removed = 0
