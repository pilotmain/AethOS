...
"""FIX 170 — durable store for mission authorization records."""
...
from aethos_core.mission_control.mission_authorization.mission_authorization_contract import (
    ...
    MAX_MISSION_AUTHORIZATION_CONTENT_LEN,
    MAX_PERSISTED_MISSION_AUTHORIZATION_RECORDS,
    MISSION_AUTHORIZATION_EXECUTABLE,
    MISSION_AUTHORIZATION_RECORD_KINDS,
    MISSION_AUTHORIZATION_RECORD_SCHEMA_VERSION,
)


def mission_authorization_records_dir() -> Path:
    root = Path(__file__).resolve().parents[3] / "data" / "mission_control_mission_authorization" / "records"
    root.mkdir(parents=True, exist_ok=True)
    return root


def clear_mission_authorization_records_for_tests() -> None:
    root = mission_authorization_records_dir()
    if root.exists():
...
def _record_path(record_id: str) -> Path:
    ...
    return mission_authorization_records_dir() / f"{safe}.json"


def list_mission_authorization_records(
    *,
    session_id: str | None = None,
    plan_id: str | None = None,
    limit: int = MAX_PERSISTED_MISSION_AUTHORIZATION_RECORDS,
) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    paths = sorted(mission_authorization_records_dir().glob("*.json"), key=lambda p: p.stat().st_mtime)
    for path in paths:
        try:
...
def append_mission_authorization_record(
    *,
    session_id: str,
    kind: str,
    content: str,
    ...
    if kind_norm not in MISSION_AUTHORIZATION_RECORD_KINDS:
    ...
    if not text:
        ...
        return None, blockers
    if len(text) > MAX_MISSION_AUTHORIZATION_CONTENT_LEN:
    ...
    record: dict[str, Any] = {
        ...
        "record_id": f"mauth-{uuid.uuid4().hex[:12]}",
...
def _prune_old_records(*, keep: int = MAX_PERSISTED_MISSION_AUTHORIZATION_RECORDS) -> int:
    paths = sorted(
        mission_authorization_records_dir().glob("*.json"),
