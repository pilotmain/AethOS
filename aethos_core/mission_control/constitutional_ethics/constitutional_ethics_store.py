...
"""FIX 159 — durable store for constitutional ethics records."""
...
from aethos_core.mission_control.constitutional_ethics.constitutional_ethics_contract import (
    ...
    MAX_ETHICS_CONTENT_LEN,
    MAX_PERSISTED_ETHICS_RECORDS,
)


def constitutional_ethics_records_dir() -> Path:
    root = (
...
def clear_constitutional_ethics_records_for_tests() -> None:
    root = constitutional_ethics_records_dir()
    if root.exists():
...
def _record_path(record_id: str) -> Path:
    ...
    return constitutional_ethics_records_dir() / f"{safe}.json"


def list_constitutional_ethics_records(
    *,
    session_id: str | None = None,
    plan_id: str | None = None,
    limit: int = MAX_PERSISTED_ETHICS_RECORDS,
) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    paths = sorted(constitutional_ethics_records_dir().glob("*.json"), key=lambda p: p.stat().st_mtime)
    for path in paths:
        try:
...
def append_constitutional_ethics_record(
    *,
    session_id: str,
    kind: str,
    content: str,
    ...
    if kind_norm not in ETHICS_RECORD_KINDS:
    ...
    if not text:
        ...
        return None, blockers
    if len(text) > MAX_ETHICS_CONTENT_LEN:
    ...
    record: dict[str, Any] = {
        ...
        "record_id": f"ethics-{uuid.uuid4().hex[:12]}",
...
def _prune_old_records(*, keep: int = MAX_PERSISTED_ETHICS_RECORDS) -> int:
    paths = sorted(constitutional_ethics_records_dir().glob("*.json"), key=lambda p: p.stat().st_mtime, reverse=True)
    removed = 0
