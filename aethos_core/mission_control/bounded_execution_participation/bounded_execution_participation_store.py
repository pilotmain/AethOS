...
"""FIX 171 — durable store for bounded execution participation records."""
...
from aethos_core.mission_control.bounded_execution_participation.bounded_execution_participation_contract import (
    ...
    BOUNDED_EXECUTION_PARTICIPATION_RECORD_KINDS,
    BOUNDED_EXECUTION_PARTICIPATION_RECORD_SCHEMA_VERSION,
    GOVERNANCE_MUTATION_PERFORMED_FIX_171,
    MAX_BOUNDED_EXECUTION_PARTICIPATION_CONTENT_LEN,
    MAX_PERSISTED_BOUNDED_EXECUTION_PARTICIPATION_RECORDS,
)


def bounded_execution_participation_records_dir() -> Path:
    root = (
...
def clear_bounded_execution_participation_records_for_tests() -> None:
    root = bounded_execution_participation_records_dir()
    if root.exists():
...
def _record_path(record_id: str) -> Path:
    ...
    return bounded_execution_participation_records_dir() / f"{safe}.json"


def list_bounded_execution_participation_records(
    *,
    session_id: str | None = None,
    plan_id: str | None = None,
    limit: int = MAX_PERSISTED_BOUNDED_EXECUTION_PARTICIPATION_RECORDS,
) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    paths = sorted(bounded_execution_participation_records_dir().glob("*.json"), key=lambda p: p.stat().st_mtime)
    for path in paths:
        try:
...
def append_bounded_execution_participation_record(
    *,
    session_id: str,
    kind: str,
    content: str,
    ...
    if kind_norm not in BOUNDED_EXECUTION_PARTICIPATION_RECORD_KINDS:
    ...
    if not text:
        ...
        return None, blockers
    if len(text) > MAX_BOUNDED_EXECUTION_PARTICIPATION_CONTENT_LEN:
    ...
    record: dict[str, Any] = {
        ...
        "record_id": f"bepart-{uuid.uuid4().hex[:12]}",
...
def _prune_old_records(*, keep: int = MAX_PERSISTED_BOUNDED_EXECUTION_PARTICIPATION_RECORDS) -> int:
    paths = sorted(
        bounded_execution_participation_records_dir().glob("*.json"),
