...
"""FIX 165 — durable store for mission planning deliberation records."""
...
def mission_planning_deliberation_records_dir() -> Path:
    root = (
...
def clear_mission_planning_deliberation_records_for_tests() -> None:
    root = mission_planning_deliberation_records_dir()
    if root.exists():
...
def _record_path(record_id: str) -> Path:
    ...
    return mission_planning_deliberation_records_dir() / f"{safe}.json"


def list_mission_planning_deliberation_records(
    *,
    ...
    paths = sorted(mission_planning_deliberation_records_dir().glob("*.json"), key=lambda p: p.stat().st_mtime)
    for path in paths:
        try:
...
def append_mission_planning_deliberation_record(
    *,
    session_id: str,
    kind: str,
    content: str,
    ...
    record: dict[str, Any] = {
        ...
        "record_id": f"delib-{uuid.uuid4().hex[:12]}",
...
def _prune_old_records(*, keep: int = MAX_PERSISTED_DELIBERATION_RECORDS) -> int:
    paths = sorted(
        mission_planning_deliberation_records_dir().glob("*.json"),
