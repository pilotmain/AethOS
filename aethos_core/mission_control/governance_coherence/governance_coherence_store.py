...
"""FIX 153 — durable store for governance coherence records."""
...
from aethos_core.mission_control.governance_coherence.governance_coherence_contract import (
    ...
    MAX_COHERENCE_CONTENT_LEN,
    MAX_PERSISTED_COHERENCE_RECORDS,
)


def coherence_records_dir() -> Path:
    root = Path(__file__).resolve().parents[3] / "data" / "mission_control_governance_coherence" / "records"
    root.mkdir(parents=True, exist_ok=True)
    return root


def clear_governance_coherence_records_for_tests() -> None:
    root = coherence_records_dir()
    if root.exists():
...
def _record_path(record_id: str) -> Path:
    ...
    return coherence_records_dir() / f"{safe}.json"


def list_governance_coherence_records(
    *,
    session_id: str | None = None,
    plan_id: str | None = None,
    limit: int = MAX_PERSISTED_COHERENCE_RECORDS,
) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    paths = sorted(coherence_records_dir().glob("*.json"), key=lambda p: p.stat().st_mtime)
    for path in paths:
        try:
...
def append_governance_coherence_record(
    *,
    session_id: str,
    kind: str,
    content: str,
    ...
    if kind_norm not in COHERENCE_RECORD_KINDS:
    ...
    if not text:
        ...
        return None, blockers
    if len(text) > MAX_COHERENCE_CONTENT_LEN:
    ...
    record: dict[str, Any] = {
        ...
        "record_id": f"gch-{uuid.uuid4().hex[:12]}",
...
def _prune_old_records(*, keep: int = MAX_PERSISTED_COHERENCE_RECORDS) -> int:
    paths = sorted(coherence_records_dir().glob("*.json"), key=lambda p: p.stat().st_mtime, reverse=True)
    removed = 0
