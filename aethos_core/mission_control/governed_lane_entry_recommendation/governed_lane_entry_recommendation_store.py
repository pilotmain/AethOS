...
"""FIX 174 — durable store for governed lane entry recommendation records."""
...
from aethos_core.mission_control.governed_lane_entry_recommendation.governed_lane_entry_recommendation_contract import (
    ...
    GOVERNED_LANE_ENTRY_RECOMMENDATION_RECORD_KINDS,
    GOVERNED_LANE_ENTRY_RECOMMENDATION_RECORD_SCHEMA_VERSION,
    GOVERNANCE_MUTATION_PERFORMED_FIX_174,
    MAX_GOVERNED_LANE_ENTRY_RECOMMENDATION_CONTENT_LEN,
    MAX_PERSISTED_GOVERNED_LANE_ENTRY_RECOMMENDATION_RECORDS,
)


def governed_lane_entry_recommendation_records_dir() -> Path:
    root = (
...
def clear_governed_lane_entry_recommendation_records_for_tests() -> None:
    root = governed_lane_entry_recommendation_records_dir()
    if root.exists():
...
def _record_path(record_id: str) -> Path:
    ...
    return governed_lane_entry_recommendation_records_dir() / f"{safe}.json"


def list_governed_lane_entry_recommendation_records(
    *,
    session_id: str | None = None,
    plan_id: str | None = None,
    limit: int = MAX_PERSISTED_GOVERNED_LANE_ENTRY_RECOMMENDATION_RECORDS,
) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    paths = sorted(
        governed_lane_entry_recommendation_records_dir().glob("*.json"),
...
def append_governed_lane_entry_recommendation_record(
    *,
    session_id: str,
    kind: str,
    content: str,
    ...
    if kind_norm not in GOVERNED_LANE_ENTRY_RECOMMENDATION_RECORD_KINDS:
    ...
    if not text:
        ...
        return None, blockers
    if len(text) > MAX_GOVERNED_LANE_ENTRY_RECOMMENDATION_CONTENT_LEN:
    ...
    record: dict[str, Any] = {
        ...
        "record_id": f"gler-{uuid.uuid4().hex[:12]}",
...
def _prune_old_records(*, keep: int = MAX_PERSISTED_GOVERNED_LANE_ENTRY_RECOMMENDATION_RECORDS) -> int:
    paths = sorted(
        governed_lane_entry_recommendation_records_dir().glob("*.json"),
        key=lambda p: p.stat().st_mtime,
        reverse=True,
