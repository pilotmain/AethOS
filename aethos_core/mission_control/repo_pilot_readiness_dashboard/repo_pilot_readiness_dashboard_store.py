...
"""FIX 182 — durable store for repo pilot readiness dashboard."""
...
from aethos_core.mission_control.repo_pilot_readiness_dashboard.repo_pilot_readiness_dashboard_contract import (
    ...
    MAX_PERSISTED_REPO_PILOT_READINESS_DASHBOARD_RECORDS,
    MAX_REPO_PILOT_READINESS_DASHBOARD_CONTENT_LEN,
    REPO_PILOT_READINESS_DASHBOARD_EXECUTABLE,
    REPO_PILOT_READINESS_DASHBOARD_ORIGIN,
    REPO_PILOT_READINESS_DASHBOARD_RECORD_KINDS,
    REPO_PILOT_READINESS_DASHBOARD_RECORD_SCHEMA_VERSION,
)


def repo_pilot_readiness_dashboard_records_dir() -> Path:
    root = (
...
def clear_repo_pilot_readiness_dashboard_records_for_tests() -> None:
    root = repo_pilot_readiness_dashboard_records_dir()
    if root.exists():
        for child in root.glob("*.json"):
            child.unlink(missing_ok=True)


def list_repo_pilot_readiness_dashboard_records(
    *,
    session_id: str | None = None,
    plan_id: str | None = None,
    limit: int = MAX_PERSISTED_REPO_PILOT_READINESS_DASHBOARD_RECORDS,
) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    paths = sorted(
        repo_pilot_readiness_dashboard_records_dir().glob("*.json"),
...
def append_repo_pilot_readiness_dashboard_record(
    *,
    session_id: str,
    kind: str,
    content: str,
    ...
    if kind_norm not in REPO_PILOT_READINESS_DASHBOARD_RECORD_KINDS:
    ...
    if not text:
        ...
        return None, blockers
    if len(text) > MAX_REPO_PILOT_READINESS_DASHBOARD_CONTENT_LEN:
    ...
    record: dict[str, Any] = {
        ...
        "record_id": f"rprd-{uuid.uuid4().hex[:12]}",
        ...
        "readiness_dashboard_origin": REPO_PILOT_READINESS_DASHBOARD_ORIGIN,
    }
    path = repo_pilot_readiness_dashboard_records_dir() / f"{record['record_id']}.json"
    path.write_text(json.dumps(record, indent=2), encoding="utf-8")
    _prune_old_records()
    return record, blockers


def _prune_old_records(*, keep: int = MAX_PERSISTED_REPO_PILOT_READINESS_DASHBOARD_RECORDS) -> int:
    paths = sorted(
        repo_pilot_readiness_dashboard_records_dir().glob("*.json"),
        key=lambda p: p.stat().st_mtime,
        reverse=True,
