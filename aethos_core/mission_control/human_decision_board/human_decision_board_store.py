...
"""FIX 166 — durable store for human decision board records."""
...
from aethos_core.mission_control.human_decision_board.human_decision_board_contract import (
    ...
    MAX_DECISION_CONTENT_LEN,
    MAX_PERSISTED_DECISION_RECORDS,
)


def human_decision_board_records_dir() -> Path:
    root = (
...
def clear_human_decision_board_records_for_tests() -> None:
    root = human_decision_board_records_dir()
    if root.exists():
...
def _record_path(record_id: str) -> Path:
    ...
    return human_decision_board_records_dir() / f"{safe}.json"


def list_human_decision_board_records(
    *,
    session_id: str | None = None,
    plan_id: str | None = None,
    limit: int = MAX_PERSISTED_DECISION_RECORDS,
) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    paths = sorted(human_decision_board_records_dir().glob("*.json"), key=lambda p: p.stat().st_mtime)
    for path in paths:
        try:
...
def append_human_decision_board_record(
    *,
    session_id: str,
    kind: str,
    content: str,
    ...
    if kind_norm not in DECISION_RECORD_KINDS:
    ...
    if not text:
        ...
        return None, blockers
    if len(text) > MAX_DECISION_CONTENT_LEN:
    ...
    record: dict[str, Any] = {
        ...
        "record_id": f"decision-{uuid.uuid4().hex[:12]}",
...
def _prune_old_records(*, keep: int = MAX_PERSISTED_DECISION_RECORDS) -> int:
    paths = sorted(human_decision_board_records_dir().glob("*.json"), key=lambda p: p.stat().st_mtime, reverse=True)
    removed = 0
