...
"""FIX 152 — durable store for governance policy interpretation records."""
...
from aethos_core.mission_control.governance_policy_interpretation.governance_policy_interpretation_contract import (
    ...
    MAX_INTERPRETATION_CONTENT_LEN,
    MAX_PERSISTED_INTERPRETATION_RECORDS,
)


def interpretation_records_dir() -> Path:
    root = (
...
def clear_governance_policy_interpretation_records_for_tests() -> None:
    root = interpretation_records_dir()
    if root.exists():
...
def _record_path(record_id: str) -> Path:
    ...
    return interpretation_records_dir() / f"{safe}.json"


def list_governance_policy_interpretation_records(
    *,
    session_id: str | None = None,
    plan_id: str | None = None,
    limit: int = MAX_PERSISTED_INTERPRETATION_RECORDS,
) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    paths = sorted(interpretation_records_dir().glob("*.json"), key=lambda p: p.stat().st_mtime)
    for path in paths:
        try:
...
def append_governance_policy_interpretation_record(
    *,
    session_id: str,
    kind: str,
    content: str,
    ...
    if kind_norm not in INTERPRETATION_RECORD_KINDS:
    ...
    if not text:
        ...
        return None, blockers
    if len(text) > MAX_INTERPRETATION_CONTENT_LEN:
    ...
    record: dict[str, Any] = {
        ...
        "record_id": f"gpi-{uuid.uuid4().hex[:12]}",
...
def _prune_old_records(*, keep: int = MAX_PERSISTED_INTERPRETATION_RECORDS) -> int:
    paths = sorted(interpretation_records_dir().glob("*.json"), key=lambda p: p.stat().st_mtime, reverse=True)
    removed = 0
