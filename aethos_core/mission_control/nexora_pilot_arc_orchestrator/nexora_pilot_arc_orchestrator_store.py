...
"""FIX 195 — durable store for Nexora pilot arc orchestrator."""
...
from aethos_core.mission_control.nexora_pilot_arc_orchestrator.nexora_pilot_arc_orchestrator_contract import (
    ...
    MAX_NEXORA_PILOT_ARC_ORCHESTRATOR_CONTENT_LEN,
    MAX_PERSISTED_NEXORA_PILOT_ARC_ORCHESTRATOR_RECORDS,
    NEXORA_DEFAULT_REPO_ISSUE,
    NEXORA_PILOT_ARC_ORCHESTRATOR_EXECUTABLE,
    NEXORA_PILOT_ARC_ORCHESTRATOR_ORIGIN,
    NEXORA_PILOT_ARC_ORCHESTRATOR_RECORD_KINDS,
...
def nexora_pilot_arc_orchestrator_records_dir() -> Path:
    root = (
...
def clear_nexora_pilot_arc_orchestrator_records_for_tests() -> None:
    root = nexora_pilot_arc_orchestrator_records_dir()
    if root.exists():
        for child in root.glob("*.json"):
            child.unlink(missing_ok=True)


def list_nexora_pilot_arc_orchestrator_records(
    *,
    limit: int = MAX_PERSISTED_NEXORA_PILOT_ARC_ORCHESTRATOR_RECORDS,
) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    paths = sorted(
        nexora_pilot_arc_orchestrator_records_dir().glob("*.json"),
...
def registered_repo_issue() -> str:
    for record in reversed(list_nexora_pilot_arc_orchestrator_records()):
    ...
    return NEXORA_DEFAULT_REPO_ISSUE
...
def append_nexora_pilot_arc_orchestrator_record(
    *,
    session_id: str,
    kind: str,
    content: str,
    ...
    if kind_norm not in NEXORA_PILOT_ARC_ORCHESTRATOR_RECORD_KINDS:
    ...
    if not text and kind_norm != "pilot_arc_transition":
        ...
        return None, blockers
    if len(text) > MAX_NEXORA_PILOT_ARC_ORCHESTRATOR_CONTENT_LEN:
    ...
    record: dict[str, Any] = {
        ...
        "record_id": f"npao-{uuid.uuid4().hex[:12]}",
        ...
        "origin": NEXORA_PILOT_ARC_ORCHESTRATOR_ORIGIN,
    }
    path = nexora_pilot_arc_orchestrator_records_dir() / f"{record['record_id']}.json"
    path.write_text(json.dumps(record, indent=2), encoding="utf-8")
    _prune_old_records()
    return record, blockers


def _prune_old_records(*, keep: int = MAX_PERSISTED_NEXORA_PILOT_ARC_ORCHESTRATOR_RECORDS) -> int:
    paths = sorted(
        nexora_pilot_arc_orchestrator_records_dir().glob("*.json"),
        key=lambda p: p.stat().st_mtime,
        reverse=True,
