...
"""FIX 157 — durable store for institutional external relations records."""
...
from aethos_core.mission_control.institutional_external_relations.institutional_external_relations_contract import (
    ...
    MAX_EXTERNAL_RELATIONS_CONTENT_LEN,
    MAX_PERSISTED_EXTERNAL_RELATIONS_RECORDS,
)


def external_relations_records_dir() -> Path:
    root = (
...
def clear_institutional_external_relations_records_for_tests() -> None:
    root = external_relations_records_dir()
    if root.exists():
...
def _record_path(record_id: str) -> Path:
    ...
    return external_relations_records_dir() / f"{safe}.json"


def list_institutional_external_relations_records(
    *,
    session_id: str | None = None,
    plan_id: str | None = None,
    limit: int = MAX_PERSISTED_EXTERNAL_RELATIONS_RECORDS,
) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    paths = sorted(external_relations_records_dir().glob("*.json"), key=lambda p: p.stat().st_mtime)
    for path in paths:
        try:
...
def append_institutional_external_relations_record(
    *,
    session_id: str,
    kind: str,
    content: str,
    ...
    if kind_norm not in EXTERNAL_RELATIONS_RECORD_KINDS:
    ...
    if not text:
        ...
        return None, blockers
    if len(text) > MAX_EXTERNAL_RELATIONS_CONTENT_LEN:
    ...
    record: dict[str, Any] = {
        ...
        "record_id": f"ier-{uuid.uuid4().hex[:12]}",
...
def _prune_old_records(*, keep: int = MAX_PERSISTED_EXTERNAL_RELATIONS_RECORDS) -> int:
    paths = sorted(external_relations_records_dir().glob("*.json"), key=lambda p: p.stat().st_mtime, reverse=True)
    removed = 0
