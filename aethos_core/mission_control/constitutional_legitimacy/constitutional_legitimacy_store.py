...
"""FIX 161 — durable store for constitutional legitimacy records."""
...
from aethos_core.mission_control.constitutional_legitimacy.constitutional_legitimacy_contract import (
    ...
    MAX_LEGITIMACY_CONTENT_LEN,
    MAX_PERSISTED_LEGITIMACY_RECORDS,
)


def constitutional_legitimacy_records_dir() -> Path:
    root = (
...
def clear_constitutional_legitimacy_records_for_tests() -> None:
    root = constitutional_legitimacy_records_dir()
    if root.exists():
...
def _record_path(record_id: str) -> Path:
    ...
    return constitutional_legitimacy_records_dir() / f"{safe}.json"


def list_constitutional_legitimacy_records(
    *,
    session_id: str | None = None,
    plan_id: str | None = None,
    limit: int = MAX_PERSISTED_LEGITIMACY_RECORDS,
) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    paths = sorted(constitutional_legitimacy_records_dir().glob("*.json"), key=lambda p: p.stat().st_mtime)
    for path in paths:
        try:
...
def append_constitutional_legitimacy_record(
    *,
    session_id: str,
    kind: str,
    content: str,
    ...
    if kind_norm not in LEGITIMACY_RECORD_KINDS:
    ...
    if not text:
        ...
        return None, blockers
    if len(text) > MAX_LEGITIMACY_CONTENT_LEN:
    ...
    record: dict[str, Any] = {
        ...
        "record_id": f"legit-{uuid.uuid4().hex[:12]}",
...
def _prune_old_records(*, keep: int = MAX_PERSISTED_LEGITIMACY_RECORDS) -> int:
    paths = sorted(constitutional_legitimacy_records_dir().glob("*.json"), key=lambda p: p.stat().st_mtime, reverse=True)
    removed = 0
