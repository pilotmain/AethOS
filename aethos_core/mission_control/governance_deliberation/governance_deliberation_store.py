...
"""FIX 148 — durable store for governance deliberation records."""
...
from aethos_core.mission_control.governance_deliberation.governance_deliberation_contract import (
    ...
    MAX_DELIBERATION_CONTENT_LEN,
    MAX_PERSISTED_DELIBERATION_RECORDS,
)


def deliberation_records_dir() -> Path:
    root = Path(__file__).resolve().parents[3] / "data" / "mission_control_governance_deliberation" / "records"
    root.mkdir(parents=True, exist_ok=True)
    return root


def clear_governance_deliberation_records_for_tests() -> None:
    root = deliberation_records_dir()
    if root.exists():
...
def _record_path(record_id: str) -> Path:
    ...
    return deliberation_records_dir() / f"{safe}.json"


def list_governance_deliberation_records(
    *,
    session_id: str | None = None,
    plan_id: str | None = None,
    limit: int = MAX_PERSISTED_DELIBERATION_RECORDS,
) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    paths = sorted(deliberation_records_dir().glob("*.json"), key=lambda p: p.stat().st_mtime)
    for path in paths:
        try:
...
def append_governance_deliberation_record(
    *,
    session_id: str,
    kind: str,
    content: str,
    ...
    if kind_norm not in DELIBERATION_RECORD_KINDS:
    ...
    if not text:
        ...
        return None, blockers
    if len(text) > MAX_DELIBERATION_CONTENT_LEN:
    ...
    record: dict[str, Any] = {
        ...
        "record_id": f"gdr-{uuid.uuid4().hex[:12]}",
...
def _prune_old_records(*, keep: int = MAX_PERSISTED_DELIBERATION_RECORDS) -> int:
    paths = sorted(deliberation_records_dir().glob("*.json"), key=lambda p: p.stat().st_mtime, reverse=True)
    removed = 0
