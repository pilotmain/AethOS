...
"""FIX 172 — durable store for governed task execution coordination records."""
...
from aethos_core.mission_control.governed_task_execution_coordination.governed_task_execution_coordination_contract import (
    ...
    GOVERNED_TASK_EXECUTION_COORDINATION_RECORD_KINDS,
    GOVERNED_TASK_EXECUTION_COORDINATION_RECORD_SCHEMA_VERSION,
    MAX_GOVERNED_TASK_EXECUTION_COORDINATION_CONTENT_LEN,
    MAX_PERSISTED_GOVERNED_TASK_EXECUTION_COORDINATION_RECORDS,
)


def governed_task_execution_coordination_records_dir() -> Path:
    root = (
...
def clear_governed_task_execution_coordination_records_for_tests() -> None:
    root = governed_task_execution_coordination_records_dir()
    if root.exists():
...
def _record_path(record_id: str) -> Path:
    ...
    return governed_task_execution_coordination_records_dir() / f"{safe}.json"


def list_governed_task_execution_coordination_records(
    *,
    session_id: str | None = None,
    plan_id: str | None = None,
    limit: int = MAX_PERSISTED_GOVERNED_TASK_EXECUTION_COORDINATION_RECORDS,
) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    paths = sorted(
        governed_task_execution_coordination_records_dir().glob("*.json"),
...
def append_governed_task_execution_coordination_record(
    *,
    session_id: str,
    kind: str,
    content: str,
    ...
    if kind_norm not in GOVERNED_TASK_EXECUTION_COORDINATION_RECORD_KINDS:
    ...
    if not text:
        ...
        return None, blockers
    if len(text) > MAX_GOVERNED_TASK_EXECUTION_COORDINATION_CONTENT_LEN:
    ...
    record: dict[str, Any] = {
        ...
        "record_id": f"gtexec-{uuid.uuid4().hex[:12]}",
...
def _prune_old_records(*, keep: int = MAX_PERSISTED_GOVERNED_TASK_EXECUTION_COORDINATION_RECORDS) -> int:
    paths = sorted(
        governed_task_execution_coordination_records_dir().glob("*.json"),
        key=lambda p: p.stat().st_mtime,
        reverse=True,
