...
"""FIX 173 — durable store for gate-routed package outcome review records."""
...
from aethos_core.mission_control.gate_routed_package_outcome_review.gate_routed_package_outcome_review_contract import (
    ...
    GATE_ROUTED_PACKAGE_OUTCOME_REVIEW_RECORD_KINDS,
    GATE_ROUTED_PACKAGE_OUTCOME_REVIEW_RECORD_SCHEMA_VERSION,
    GOVERNANCE_MUTATION_PERFORMED_FIX_173,
    MAX_GATE_ROUTED_PACKAGE_OUTCOME_REVIEW_CONTENT_LEN,
    MAX_PERSISTED_GATE_ROUTED_PACKAGE_OUTCOME_REVIEW_RECORDS,
)


def gate_routed_package_outcome_review_records_dir() -> Path:
    root = (
...
def clear_gate_routed_package_outcome_review_records_for_tests() -> None:
    root = gate_routed_package_outcome_review_records_dir()
    if root.exists():
...
def _record_path(record_id: str) -> Path:
    ...
    return gate_routed_package_outcome_review_records_dir() / f"{safe}.json"


def list_gate_routed_package_outcome_review_records(
    *,
    session_id: str | None = None,
    plan_id: str | None = None,
    limit: int = MAX_PERSISTED_GATE_ROUTED_PACKAGE_OUTCOME_REVIEW_RECORDS,
) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    paths = sorted(
        gate_routed_package_outcome_review_records_dir().glob("*.json"),
...
def append_gate_routed_package_outcome_review_record(
    *,
    session_id: str,
    kind: str,
    content: str,
    ...
    if kind_norm not in GATE_ROUTED_PACKAGE_OUTCOME_REVIEW_RECORD_KINDS:
    ...
    if not text:
        ...
        return None, blockers
    if len(text) > MAX_GATE_ROUTED_PACKAGE_OUTCOME_REVIEW_CONTENT_LEN:
    ...
    record: dict[str, Any] = {
        ...
        "record_id": f"gtrev-{uuid.uuid4().hex[:12]}",
...
def _prune_old_records(*, keep: int = MAX_PERSISTED_GATE_ROUTED_PACKAGE_OUTCOME_REVIEW_RECORDS) -> int:
    paths = sorted(
        gate_routed_package_outcome_review_records_dir().glob("*.json"),
        key=lambda p: p.stat().st_mtime,
        reverse=True,
