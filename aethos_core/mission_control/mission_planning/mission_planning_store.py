...
"""FIX 164 — durable store for mission planning records."""
...
from aethos_core.mission_control.mission_planning.mission_planning_contract import (
    ...
    MAX_PERSISTED_PLANNING_RECORDS,
    MAX_PLANNING_CONTENT_LEN,
...
def mission_planning_records_dir() -> Path:
    root = (
...
def clear_mission_planning_records_for_tests() -> None:
    root = mission_planning_records_dir()
    if root.exists():
...
def _record_path(record_id: str) -> Path:
    ...
    return mission_planning_records_dir() / f"{safe}.json"


def list_mission_planning_records(
    *,
    session_id: str | None = None,
    plan_id: str | None = None,
    limit: int = MAX_PERSISTED_PLANNING_RECORDS,
) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    paths = sorted(mission_planning_records_dir().glob("*.json"), key=lambda p: p.stat().st_mtime)
    for path in paths:
        try:
...
def append_mission_planning_record(
    *,
    session_id: str,
    kind: str,
    content: str,
    ...
    if kind_norm not in PLANNING_RECORD_KINDS:
    ...
    if not text:
        ...
        return None, blockers
    if len(text) > MAX_PLANNING_CONTENT_LEN:
    ...
    record: dict[str, Any] = {
        ...
        "record_id": f"plan-{uuid.uuid4().hex[:12]}",
...
def _prune_old_records(*, keep: int = MAX_PERSISTED_PLANNING_RECORDS) -> int:
    paths = sorted(mission_planning_records_dir().glob("*.json"), key=lambda p: p.stat().st_mtime, reverse=True)
    removed = 0
