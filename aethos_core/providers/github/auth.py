...
"""GitHub auth adapter — personal access token via credential vault."""
...
class GitHubAuthAdapter(AuthAdapter):
    provider = "github"
    ...
    def resolve_best_auth_method(self, *, operation: str = "read_repos") -> dict[str, Any]:
        _ = operation
        ...
        if not cred:
            return {
                "method": None,
                "credential_id": None,
                "detail": "No GitHub token configured. Add one in Mission Control → Advanced settings → Credentials.",
            }
