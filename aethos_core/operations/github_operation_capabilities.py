...
"""GitHub operation capability helpers."""
...
def _github_cap(operation_type: str):
    return ProviderRegistry.get_operation_capability("github", operation_type)


def is_api_capable(operation_type: str) -> bool:
    cap = _github_cap(operation_type)
...
def resolve_execution_auth(operation_type: str, params: dict[str, Any]) -> dict[str, Any]:
    ...
    if not credential_id and is_api_capable(operation_type):
        ...
        if resolved.get("method") == "api_token":
            ...
            auth_label = auth_method_label_for_provider("github", "api_token")
    if not auth_label and auth_method:
        auth_label = auth_method_label_for_provider("github", auth_method)
    return {
...
def preflight_capability_metadata(operation_type: str) -> dict[str, Any]:
    ...
    return {
        "auth_method": auth_method,
        "auth_method_label": auth_method_label_for_provider("github", auth_method),
